...
B738DR_B738DR_below_gs_pilot	= create_dataref("laminar/B738/push_button/below_gs_pilot_pos", "number")
B738DR_B738DR_below_gs_copilot	= create_dataref("laminar/B738/push_button/below_gs_copilot_pos", "number")
...
B738_win_l						= create_dataref("laminar/B738/effect/windowL", "number", B738_win_l_DRhandler)
...
function B738_below_gs_disable_pilot_CMDhandler(phase, duration)
	if phase == 0 then
		B738DR_B738DR_below_gs_pilot = 1
		if B738DR_below_gs ~= 0 then
			B738DR_below_gs_disable = 1
		end
	elseif phase == 2 then
		B738DR_B738DR_below_gs_pilot = 0
	end
end
...
function B738_below_gs_disable_copilot_CMDhandler(phase, duration)
	if phase == 0 then
		B738DR_B738DR_below_gs_copilot = 1
		if B738DR_below_gs ~= 0 then
			B738DR_below_gs_disable = 1
		end
	elseif phase == 2 then
		B738DR_B738DR_below_gs_copilot = 0
	end
end
...
--	if l_pack == 0 and B738DR_isolation_valve_pos == 1 and r_pack > 0.95 then	-- L PACK: OFF, ISO valve: OPEN, R PACK: AUTO or HIGH
	...
	apu_start_eng2 = 0
--	if l_pack == 0 and B738DR_isolation_valve_pos == 1 and r_pack == 0 then		-- L PACK: OFF, ISO valve: OPEN, R PACK: OFF
...
local rndm_max_eng1_bleed_psi   = math.random(52, 56) + math.random()	-- add 5 psi
local rndm_max_eng2_bleed_psi   = math.random(53, 57) + math.random()	-- add 5 psi
...
local apu_start_time    = math.random(20, 40)	-- start time APU
...
function B738_bleed_air_valves()
	...
	if simDR_apu_bleed_fail == 1 then												-- APU damaged or fail
...
function B738_bleed_air_duct_pressure()
	...
	-- LEFT DUCT
		...
		l_duct = l_duct_in
		if l_duct >= bleed_valve_min_act_press then
			l_duct = l_duct + (2.5 * (1 - B738bleedAir.l_pack.bleed_air_valve.pos))
		end
	
	-- RIGHT DUCT
		r_duct_in = B738bleedAir.engine2.psi * B738bleedAir.engine2.bleed_air_valve.pos
		r_duct = r_duct_in
		if r_duct >= bleed_valve_min_act_press then
			r_duct = r_duct + (2.5 * (1 - B738bleedAir.r_pack.bleed_air_valve.pos))
		end
		
	-- ISOLATION VALVE
		l_duct_delta = r_duct - l_duct
		r_duct_delta = l_duct - r_duct
		if l_duct > r_duct then
		...
		if l_duct >= bleed_valve_min_act_press
		and r_duct >= bleed_valve_min_act_press then
			l_duct = l_duct + (2.5 * B738bleedAir.isolation.bleed_air_valve.pos)
			r_duct = r_duct + (2.5 * B738bleedAir.isolation.bleed_air_valve.pos)
		end

		l_duct_out_target = l_duct
		r_duct_out_target = r_duct
		l_duct_out = B738_set_anim_value(l_duct_out, l_duct_out_target, 0.0, 80.0, 2.0)
		r_duct_out = B738_set_anim_value(r_duct_out, r_duct_out_target, 0.0, 80.0, 2.0)
	...
	if dual_bleed == 1 then
		if eng1_duct > 33 or eng2_duct > 33 then
...
-- function B738_start_engine()
	...
	-- -- ENGINE 1
	-- if B738DR_engine1_starter_pos == 0 then		-- Position GRD
		-- if simDR_engine_mixture1 < 0.1 then
			-- ignition1 = 4
		-- end
		-- if simDR_engine_N2_pct1 > 56 and simDR_engine_N2_pct1 < 58 then -- release to OFF and starter off
			...
			-- B738DR_engine1_starter_pos = 1
		-- end
	-- elseif B738DR_engine1_starter_pos == 1 then		-- Position OFF
		-- if simDR_engine_mixture1 < 0.1 then
			-- if simDR_engine_N2_pct1 < 45 then
				-- ignition1 = 0
			-- end
			-- if simDR_engine_N2_pct1 > 50 and simDR_engine_N2_pct1 < 57 then
				-- ignition1 = 4
			-- end
			-- if simDR_on_ground_0 == 0 and simDR_engine_N2_pct1 < 70 then
				-- ignition1 = 4
			-- end
		-- else
			-- ignition1 = 3
		-- end
	-- elseif B738DR_engine1_starter_pos == 2 then		-- Position CNT
		-- if simDR_engine_mixture1 < 0.1 and simDR_engine_N1_pct1 > 22 then
			-- if simDR_on_ground_0 == 0 then
		...
		-- else
			-- ignition1 = 3
		-- end
	-- elseif B738DR_engine1_starter_pos == 3 then		-- Position FLT
		-- if simDR_engine_mixture1 < 0.1 then
			-- ignition1 = 4
		-- else
			-- ignition1 = 3
	...
	-- end
	-- -- ENGINE 2
	-- if B738DR_engine2_starter_pos == 0 then		-- Position GRD
		-- if simDR_engine_mixture2 < 0.1 then
			-- ignition2 = 4
		-- end
		-- if simDR_engine_N2_pct2 > 56 and simDR_engine_N2_pct2 < 58 then -- release to OFF and starter off
			...
			-- B738DR_engine2_starter_pos = 1
		-- end
	-- elseif B738DR_engine2_starter_pos == 1 then		-- Position OFF
		-- if simDR_engine_mixture2 < 0.1 then
			-- if simDR_engine_N2_pct2 < 45 then
				-- ignition2 = 0
			-- end
			-- if simDR_engine_N2_pct2 > 50 and simDR_engine_N2_pct2 < 57 then
				-- ignition2 = 4
			-- end
			-- if simDR_on_ground_0 == 0 and simDR_engine_N2_pct2 < 70 then
				-- ignition2 = 4
			-- end
		-- else
			-- ignition2 = 3
		-- end
	-- elseif B738DR_engine2_starter_pos == 2 then		-- Position CNT
		-- if simDR_engine_mixture2 < 0.1 and simDR_engine_N1_pct2 > 22 then
			-- if simDR_on_ground_0 == 1 then
		...
		-- else
			-- ignition2 = 3
		-- end
	-- elseif B738DR_engine2_starter_pos == 3 then		-- Position FLT
		-- if simDR_engine_mixture2 < 0.1 then
			-- ignition2 = 4
		-- else
			-- ignition2 = 3
...
function B738_start_engine()
	...
	elseif B738DR_engine1_starter_pos == 1 then
		...
		if simDR_engine_mixture1 > 0.5 then		-- start lever> idle
			...
			if simDR_on_ground_0 == 0 and simDR_engine_N2_pct1 < 70
			and simDR_engine_N2_pct1 > 5 then
				ignition1 = 3
			end
		end
	-- Position CNT
	...
	elseif B738DR_engine2_starter_pos == 1 then
		...
		if simDR_engine_mixture2 > 0.5 then		-- start lever> idle
			...
			if simDR_on_ground_0 == 0 and simDR_engine_N2_pct2 < 70
			and simDR_engine_N2_pct2 > 5 then 
				ignition2 = 3
			end
		end
	-- Position CNT
...
function B738_fuel_pump()
	if B738DR_fuel_tank_pos_lft1 == 1 or B738DR_fuel_tank_pos_lft2 == 1 then
--		simDR_tank_l_status = 1
		B738_tank_l_status = 1
	end
	if B738DR_fuel_tank_pos_lft1 == 0 and B738DR_fuel_tank_pos_lft2 == 0 then
--		simDR_tank_l_status = 0
		B738_tank_l_status = 0
	end
	if B738DR_fuel_tank_pos_rgt1 == 1 or B738DR_fuel_tank_pos_rgt2 == 1 then
--		simDR_tank_r_status = 1
		B738_tank_r_status = 1
	end
	if B738DR_fuel_tank_pos_rgt1 == 0 and B738DR_fuel_tank_pos_rgt2 == 0 then
...
--		simDR_tank_c_status = 1
	...
	if B738DR_fuel_tank_pos_ctr1 == 0 and B738DR_fuel_tank_pos_ctr2 == 0 then
...
function B738_apu_system2()
	...
	if apu_start_active == 1 and B738DR_apu_was_fire == 0 and simDR_apu_bleed_fail == 0 then	-- START on
		if simDR_apu_run == 0 then								-- APU not running
			if is_timer_scheduled(apu_start) == false then
				...
				run_after_time(apu_low_oil, 2.5)				-- Delay 2 second
			end
		else
			apu_start_active = 3
			apu_shutdown_active = 0
	...
	if apu_turn_off == 1 or B738DR_apu_was_fire == 1 or simDR_apu_bleed_fail == 1 then
		apu_shutdown_active = 1
		apu_start_active = 0
		B738DR_apu_bus_enable = 0
...
function B738_autobrake()
	...
	if B738DR_parking_brake_pos > 0.1
	or brake_force > (autobrake_ratio + 0.1) then
		manual_brake = 1
	end
	...
	if B738DR_thrust1_leveler > 0.3 
	and B738DR_thrust2_leveler > 0.3 then
		if simDR_reverse1_deploy < 0.05
		and simDR_reverse2_deploy < 0.05 then
			throttle_30 = 0
		end
	...
	if B738DR_autobrake_RTO_arm == 2 then
		...
		--simDR_brake = math.max(autobrake_ratio, brake_force)
		--simDR_left_brake = math.max(simDR_left_brake, autobrake_ratio)
		--simDR_right_brake = math.max(simDR_right_brake, autobrake_ratio)
...
--			or throttle_idle == 0 
			...
			--or speed_brake_old > simDR_speedbrake_ratio
			or manual_brake == 1 then
				...
				--speed_brake_old = simDR_speedbrake_ratio
			end
			--simDR_brake = math.max(autobrake_ratio, brake_force)
			--simDR_left_brake = math.max(simDR_left_brake, autobrake_ratio)
			--simDR_right_brake = math.max(simDR_right_brake, autobrake_ratio)
	...
	-- if aircraft_landing2 == 1 and throttle_idle_3s == 0 then	-- SPEED BRAKE UP
		-- simDR_speedbrake_ratio = 0.0					-- SPEED BRAKE DOWN
		-- aircraft_landing2 = 0
	-- end
	...
	or simDR_on_ground_2 == 1 then
		if B738DR_autobrake_arm == 1 then
			...
			if B738DR_autobrake_pos == 2 then
				--simDR_brake = 0.055
				autobrake_ratio = 0.075
			elseif B738DR_autobrake_pos == 3 then
				--simDR_brake = 0.11
				autobrake_ratio = 0.15
			elseif B738DR_autobrake_pos == 4 then
				--simDR_brake = 0.165
				autobrake_ratio = 0.225
			elseif B738DR_autobrake_pos == 5 then
				--simDR_brake = 0.22
				autobrake_ratio = 0.30
			end
		end
	end
...
function B738_yaw_damp()
	...
	if B738DR_irs_left_mode > 1 or B738DR_irs_right_mode > 1 then
		if B738DR_yaw_damper_switch_pos == 1 and simDR_yaw_damper_switch == 0 and yaw_damp_hydr == 1 then
			if is_timer_scheduled(yaw_damp_on) == false then
				run_after_time(yaw_damp_on, 1.5)
			end
		end
	end
	if B738DR_irs_left_mode < 2 and B738DR_irs_right_mode < 2
	and B738DR_yaw_damper_switch_pos == 1 then
...
function turn_around_state()
	if first_time > 3 and first_time < 4 then
		first_time = 4
...
function B738_tcas(ai_plane)
	...
	local mag_hdg = simDR_mag_hdg - simDR_mag_variation
	local tcas_alt = 0
...
function B738_tcas_system()
	...
	-- if B738DR_transponder_knob_pos > 0 and B738DR_EFIS_TCAS_on == 1 
	-- and simDR_map_mode_is_HSI == 0 and simDR_EFIS_mode ~= 4 then
		-- if is_timer_scheduled(B738_tcas_calc) == false then
			-- run_at_interval(B738_tcas_calc, 2)		-- calc every 2 seconds
		-- end
	...
	if B738DR_transponder_knob_pos > 0 then
		...
		-- tcas_tara_test = B738DR_tcas_box_show + B738DR_tcas_box_show2 + B738DR_tcas_box_show3 + B738DR_tcas_box_show4
		-- tcas_tara_test = tcas_tara_test + B738DR_tcas_box_show5 + B738DR_tcas_box_show6 + B738DR_tcas_box_show7 + B738DR_tcas_box_show8
		-- tcas_tara_test = tcas_tara_test + B738DR_tcas_box_show9 + B738DR_tcas_box_show10
		-- if tcas_tara_test > 0 then
			-- B738_EFIS_traffic_ra = 1
			-- B738_EFIS_traffic_ta = 0
		-- else
			-- tcas_tara_test = 0
			-- tcas_tara_test = B738DR_tcas_circle_show + B738DR_tcas_circle_show2 + B738DR_tcas_circle_show3 + B738DR_tcas_circle_show4
			-- tcas_tara_test = tcas_tara_test + B738DR_tcas_circle_show5 + B738DR_tcas_circle_show6 + B738DR_tcas_circle_show7 + B738DR_tcas_circle_show8
			-- tcas_tara_test = tcas_tara_test + B738DR_tcas_circle_show9 + B738DR_tcas_circle_show10
			-- if tcas_tara_test > 0 then
				-- B738_EFIS_traffic_ra = 0
				-- B738_EFIS_traffic_ta = 1
			-- else
		...
		-- if tcas_tara_status == 0 then
			-- B738_EFIS_traffic_ra = 0
			-- B738_EFIS_traffic_ta = 0
		-- elseif tcas_tara_status == 1 then
			-- B738_EFIS_traffic_ra = 0
			-- B738_EFIS_traffic_ta = 1
		-- elseif tcas_tara_status == 1 then
			-- B738_EFIS_traffic_ra = 1
	...
	if B738DR_EFIS_TCAS_on == 0 or simDR_map_mode_is_HSI == 1
	or simDR_EFIS_mode == 4 then
		-- if is_timer_scheduled(B738_tcas_calc) == true then
			-- stop_timer(B738_tcas_calc)
		-- end
		B738_EFIS_TCAS_show = 0
		B738_EFIS_tfc_show = 0
		B738_EFIS_ta_only_show = 0
...
--B738DR_slat2_deg		= create_dataref("laminar/B738/slat2_deploy_ratio", "number")
	...
	if flaps_deg <= 8 then
	--B738DR_slat1_deg = B738_rescale(in1, out1, in2, out2, x)
...
function B738_nose_steer()
	...
	if left_brake > 0.5 and right_brake > 0.5 then
		B738DR_parking_brake_pos = 0
	end
	...
	if B738DR_chock_status == 0 then
		...
		if B738DR_toe_brakes_ovr == 0 then
			...
			if simDR_on_ground_0 == 1 or simDR_on_ground_1 == 1
			or simDR_on_ground_2 == 1 then
				if simDR_airspeed_pilot < 40 then
					simDR_toe_brakes_ovr = 1
					nose_steer_deg_trg = B738_rescale(-1, -65, 1, 65, simDR_yoke_hdg_ratio)
					if B738DR_nosewheel == 0 then
						steer_position = simDR_steer_cmd
						steer_pos_min = -65
						steer_pos_max = 65
					else
						steer_position = simDR_yoke_hdg_ratio
						steer_pos_min = -1
						steer_pos_max = 1
					end
...
function B738_hydraulic_sys()
	...
	if B738DR_el_hydro_pumps1_switch_position == 1 then
		hyd_A_pressure_target1 = 2700
	end
	local hyd_A_pressure_target2 = 0
	if B738DR_hydro_pumps1_switch_position == 1 then
	...
	if hyd_A_pressure_target > hyd_A_pressure then
		...
		hyd_A_pressure = B738_set_anim_value(hyd_A_pressure, hyd_A_pressure_target, 0, 3500, 0.16)
	end
	B738DR_hyd_A_pressure = roundDownToIncrement(hyd_A_pressure, 10)
	
	-- HYD B pressure
	local hyd_B_pressure_target1 = 0
	if B738DR_el_hydro_pumps2_switch_position == 1 then
		hyd_B_pressure_target1 = 2700
	end
	local hyd_B_pressure_target2 = 0
	if B738DR_hydro_pumps2_switch_position == 1 then
	...
	if hyd_B_pressure_target > hyd_B_pressure then
		hyd_B_pressure = B738_set_anim_value(hyd_B_pressure, hyd_B_pressure_target, 0, 3500, 0.63)
	else
		hyd_B_pressure = B738_set_anim_value(hyd_B_pressure, hyd_B_pressure_target, 0, 3500, 0.18)
	end
	...
	if hyd_stdby_pressure_target > hyd_stdby_pressure then
		hyd_stdby_pressure = B738_set_anim_value(hyd_stdby_pressure, hyd_stdby_pressure_target, 0, 3500, 0.63)
	else
		hyd_stdby_pressure = B738_set_anim_value(hyd_stdby_pressure, hyd_stdby_pressure_target, 0, 3500, 0.18)
	end
	B738DR_hyd_stdby_pressure = roundDownToIncrement(hyd_stdby_pressure, 10)
	
	-- HYD A
	--if simDR_hydraulic_press_1 > 1000 then	-- HYD system A
	...
	-- HYD B
	--if simDR_hydraulic_press_2 > 1000 then	-- HYD system B
	...
	if B738DR_hyd_A_rud == 0 and B738DR_hyd_B_rud == 0 then
		...
		yoke_heading_ratio = B738_rescale(0, 0, 1, 2, simDR_yoke_heading_ratio) - 1
		if yoke_heading_ratio > 0.2 then
			yoke_heading_ratio = 0.2
		end
		if yoke_heading_ratio < -0.2 then
			yoke_heading_ratio = -0.2
		end
		...
		yoke_pitch_ratio = B738_rescale(0, 0, 1, 2, simDR_yoke_pitch_ratio) - 1
		if yoke_pitch_ratio > 0.2 then
			yoke_pitch_ratio = 0.2
		end
		if yoke_pitch_ratio < -0.2 then
			yoke_pitch_ratio = -0.2
		end
		...
		yoke_roll_ratio = B738_rescale(0, 0, 1, 2, simDR_yoke_roll_ratio) - 1
		if yoke_roll_ratio > 0.2 then
			yoke_roll_ratio = 0.2
		end
		if yoke_roll_ratio < -0.2 then
			yoke_roll_ratio = -0.2
...
function B738_pressurization()
	...
	local press_mode = 0
	local press_taget_alt = 0
	
	local bus1 = 0
	local bus2 = 0
	local battery_bus = 0
	
	if simDR_bus_volts1 > 10 then
		bus1 = 1
	end
	if simDR_bus_volts2 > 10 then
		bus2 = 1
	end
	if simDR_bus_volts3 > 10 then
		bus3 = 1
	end
	
	local el_bus = 0
	if bus1 == 1 or bus2 == 1 then
		el_bus = 1
	end
	B738DR_power_on_buses = el_bus
	...
	elseif press_mode < 3 then
		simDR_dump_all = 0
		simDR_dump_to_alt = 0
		if simDR_altitude_pilot <= simDR_press_max_alt then
			press_taget_alt = 8000
		else
			press_taget_alt = (simDR_altitude_pilot - simDR_press_max_alt) + 8000
			--press_taget_alt = simDR_altitude_pilot + 8000
		end
		simDR_press_set_alt = press_taget_alt
	else
		simDR_dump_all = 0
		simDR_dump_to_alt = 1
		press_taget_alt = (simDR_altitude_pilot - simDR_press_max_alt) + 8000
